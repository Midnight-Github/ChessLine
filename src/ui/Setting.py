...
class Setting(MainPageStructure):
    ...
    def createSettingsFrame(self, heading_text, row, column):
    ...
    def board(self):
        self.board_colors = ["Gray", "Blue", "Green"]

        self.board_frame = self.createSettingsFrame(heading_text="Board", row=0, column=1)

        self.board_color = ctk.CTkOptionMenu(self.board_frame, values=self.board_colors, width=90, command=self.checkChanges)
    ...
    def appearance(self):
        ...
        self.system_color_themes = ("blue", "dark-blue", "green")

        self.appearance_frame = self.createSettingsFrame(heading_text="Appearance", row=0, column=0)
        ...
        self.color_theme = ctka.labeledRadioButton(self.appearance_frame, label_text="Color theme", options=[i.capitalize() for i in self.system_color_themes], 
        row=1, column=1, padx=(0, 5), pady=(0, 5), value=self.system_color_themes.index(self.configurator.config["appearance"]["color_theme"]), command=self.checkChanges)
    
    def checkChanges(self, e=None):
        changes = [ 
    ...
    def checkRestartRequired(self):
        changes = [self.configurator.config["appearance"]["color_theme"] != self.system_color_themes[self.color_theme.get()]]
        return any(changes)
